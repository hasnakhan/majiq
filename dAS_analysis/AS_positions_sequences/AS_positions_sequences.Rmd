...
dev_stress <- events %>% filter(comp_type == "developmental") %>% 
  left_join(., pos_list, by = "event")
```
...
```{r}
png("test.png", units="in", width=8, height=4, res=300)
ggplot(dev_stress, aes(x = strand))+
...
for (i in 1:nrow(ir_cds_inc)){
  ...
  for (j in 1:nrow(sorted)){
      chr = single_gene_data[1,1]
      chr <- dplyr::filter(athal_gdna, seq.name == chr[1,1])
      chr <- dplyr::filter(athal_gdna, seq.name == chr[1,1])
  ...
  seq <- as.character(translate(DNAString(seq), if.fuzzy.codon = "X"))
