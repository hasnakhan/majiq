...
#for (i in 57:171){
  ...
  for (j in 1:nrow(sorted)){
      ...
      #chr <- dplyr::filter(athal_gdna, seq.name == chr_num[1,1])
...
t.test(ir_length$length, all_introns$length)
...
plot intron length
```{r}
png("test.png", units="in", width=8, height=3, res=300)
...
ggplot(., aes(x = length, y = fct_rev(group), fill = group, colour = group)) + 
  ...
  scale_fill_manual(values = c("orchid", "limegreen"))+
  scale_colour_manual(values = c("orchid", "limegreen"))+
