...
barplot <- vm_allcomps %>% group_by(comp) %>% 
  summarize(count = n()) %>%
...
```{r}
png("test.png", units="in", width=12, height=6, res=300)
ggplot(barplot, aes(x = comp, y = count, fill = count))+
...
go <- join(go, colour_key, by = "term_type")
...
ggplot(go, aes(x = fct_inorder(cluster), y = thingy, colour = log10(FDR), size = queryitem, label = cluster))+
  geom_point()+
...
```{r}
png("test.png", units="in", width=12, height=15, res=300)
ggplot(filtered, aes(x = fct_inorder(cluster), y = thingy, colour = log10(FDR), size = queryitem, label = cluster))+
  geom_point()+
  scale_colour_gradient(low = "blue", high = "red")+
  theme_bw()+
