...
```{r}
as_pca <- prcomp(na.omit(pca))
summary(as_pca)
as_pca$x
pca_data <- cbind(as_pca$x, inc_pca$sample, inc_pca$group)
pca_data <- as.data.frame(pca_data)
```

```{r}
ggplot(pca_data, aes(x = PC1, y = PC2, colour = V44))+
  geom_point(size = 6)
...
what if we cap it at 2 PCs?
```{r}
as_pca2 <- prcomp(pca, rank. = 2)
summary(as_pca2)
