...
all_orthologs <- do.call(rbind, lapply(files, read_delim, delim = "\t")) %>% 
  select(-c(1, 6)) %>% 
  filter(!`Gene Name` == "AT1G14270")
#filter out the A3SS event
...
```{r}
upexon_scores_test <- upexon_scores_plot %>% 
  pivot_wider(names_from = Type_mean, values_from = mean) %>% 
  filter(Type_median == "alt_med")
wilcox.test(upexon_scores_test$cons_mean, upexon_scores_test$alt_mean, paired = T, alternative = "greater") #p = 9.095e-13
```
...
ggplot(upexon_scores_plot, aes(x = position, y = mean, colour = Type_mean))+
  geom_line(size = 2)+
  #lims(y = c(0.0185, 0.0225))+
  labs(title = "Eudicot species",
       subtitle = "Upstream exon",
       y = "Mean phyloP score (3nt rolling average)",
       x = "Position relative to 5' SS")+
  theme_minimal()+
  theme(legend.title = element_blank(),
        text = element_text(size = 14))+
  scale_colour_manual(labels = c("Alternative", "Constitutive"), values = c("limegreen", "darkblue"))
dev.off()
```
...
```{r}
downexon_scores_test <- downexon_scores_plot %>% 
  pivot_wider(names_from = Type_mean, values_from = mean) %>% 
  filter(Type_median == "alt_med")
wilcox.test(downexon_scores_test$cons_mean, downexon_scores_test$alt_mean, paired = T, alternative = "greater") #p = 9.095e-13
```
...
upstream exons
```{r}
setwd("C:/Users/hasna/Documents/MSc_project/majiq/dAS_analysis/conservation/")
...
this code isn't quite up to date, a ratio barplot and italicized tree exist in image form but the code has been lost to the ether (can be re-written as needed :)
```{r}
library(ggtree)
library(tidyverse)
```
...
tree_data <- tree_data %>% mutate(alt = alt_struct/alt_ortho, const = const_struct/const_ortho) %>% 
  mutate(ratio = alt/const)
tree_data$tip.label <- factor(tree_data$tip.label, levels = c("A. thaliana", "A. lyrata", "A. halleri", "C. rubella", "B. rapa", "S. lycopersicum", "S. tuberosum", "G. max"))
...
g <- ggtree(tree, ladderize = F)+
  geom_tiplab(hjust = 1, size = 4, vjust = -.5)
ggplot(tree_data, aes(x = ratio, y = fct_rev(tip.label)))+
  geom_bar(stat = "identity", fill = "lightblue")+
  coord_cartesian(xlim = c(0.7, 1.1))+
  geom_vline(xintercept = 1, linetype = 2)+
  theme_minimal()+
  labs(x = "Ratio of conserved alternative introns to constitutive introns",
       y = "")
...
statistical testing on the ratios (Fisher's exact test with post-hoc BH correction)
no significant differences
```{r}
AlyrataFET <- data.frame(cons = c(21, 21),
                         notcons = c(0, 1),
                         row.names = c("Alt", "Const"))
fisher.test(AlyrataFET) #p = 1

AhalleriFET <- data.frame(cons = c(19, 18),
                         notcons = c(2, 3),
                         row.names = c("Alt", "Const"))
fisher.test(AhalleriFET) # p = 1

BrapaFET <- data.frame(cons = c(21, 21),
                         notcons = c(1, 1),
                         row.names = c("Alt", "Const"))
fisher.test(BrapaFET) # p = 1

CrubellaFET <- data.frame(cons = c(19, 20),
                         notcons = c(3, 2),
                         row.names = c("Alt", "Const"))
fisher.test(CrubellaFET) # p = 1

StuberosumFET <- data.frame(cons = c(13, 15),
                         notcons = c(5, 4),
                         row.names = c("Alt", "Const"))
fisher.test(StuberosumFET) # p = 0.714

SlycopersicumFET <- data.frame(cons = c(15, 16),
                         notcons = c(4, 4),
                         row.names = c("Alt", "Const"))
fisher.test(SlycopersicumFET) #p = 1

GmaxFET <- data.frame(cons = c(11, 15),
                         notcons = c(5, 2),
                         row.names = c("Alt", "Const"))
fisher.test(GmaxFET) #0.2245
```
...
upexon_scores_plot <-  upexon_scores_plot %>% pivot_longer(c(alt_med, cons_med), names_to = "Type_median", values_to = "median") %>% 
  pivot_longer(c(alt_mean, cons_mean), names_to = "Type_mean", values_to = "mean")
title <- expression(paste(italic("A. thaliana "), "accessions"))
```
...
test for significance by wilcoxon sign-rank test (basically a non-parametric paired t-test)
```{r}
upexon_scores_test <- upexon_scores_plot %>% 
  pivot_wider(names_from = Type_mean, values_from = mean) %>% 
  filter(Type_median == "alt_med")
#not gonna do paired here casue it wasn't immediately obvious to me that i should (ie wasn't my null hypothesis)
wilcox.test(upexon_scores_test$alt_mean, upexon_scores_test$cons_mean, paired = T) #p = 0.009968
```
...
ggplot(downexon_scores_plot, aes(x = position, y = mean, colour = Type_mean))+
  ...
  theme(legend.title = element_blank(),
        text = element_text(size = 14))+
  scale_y_continuous(position = "right", limits = c(0.0175, 0.0235))+
  scale_colour_manual(labels = c("Alternative", "Constitutive"), values = c("limegreen", "darkblue"))
dev.off()
```
...
downexon_scores_test <- downexon_scores_plot %>% 
  pivot_wider(names_from = Type_mean, values_from = mean) %>% 
  filter(Type_median == "alt_med")
wilcox.test(downexon_scores_test$alt_mean, downexon_scores_test$cons_mean, paired = T) #p = 2.988e-06
```
