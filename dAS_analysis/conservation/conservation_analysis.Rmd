...
upexon_scores_test <- upexon_scores_plot %>% 
  pivot_wider(names_from = Type_mean, values_from = mean) %>% 
  filter(Type_median == "alt_med")
wilcox.test(upexon_scores_test$alt_mean, upexon_scores_test$cons_mean, paired = T, alternative = "greater") #p = 0.004984
```
...
downexon_scores_test <- downexon_scores_plot %>% 
  pivot_wider(names_from = Type_mean, values_from = mean) %>% 
  filter(Type_median == "alt_med")
wilcox.test(downexon_scores_test$alt_mean, downexon_scores_test$cons_mean, paired = T, alternative = "greater") #p = 1.494e-06
```
