...
twoway <- aov(all_taus$exp_profile ~ all_taus$group * all_taus$max)
summary(twoway)
ggplot(all_taus, aes(x = max, y = exp_profile))+
  geom_point()+
  lims(x = c(0, 500))+
  geom_smooth(method = "lm")
```

the line plot above isn't terribly informative, since the significant interaction term suggests that each group's tau responds differently to changes in TPM. we can visulaize this with an interaction plot:
```{r}
ggplot(all_taus, aes(x = max, y = exp_profile, colour = group))+
  geom_point()+
  ...
  labs(y = "Tissue specificity index",
       x = "Max expression level (RPKM)")
```
these trendlines aren't terribly different, I'm ok with that
